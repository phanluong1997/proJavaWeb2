 ...
 * @author Admin
 */
public class GiayListServlet extends HttpServlet {
...
"            </aside>\n" +
"            <section>");
             out.println("<h2> Danh sách giày bán chạy năm 2019</h2>");
             out.println("<input type=\"button\" value=\"Thêm mới\" onclick=\"location.href='GiayAddServlet';\">");
             
             DAOgiay dao = new DAOgiay();
             dao.ReadAll();
            ArrayList<giay> list=dao.getList();
            
            out.println("<form action='GiayEditServlet' method= 'post'>");
             out.println("<table>");
             out.print("<thead>"
             + "<th>IDGiay</th>"
             + "<th>TênGiày</th>"
            ...
            for (giay s : list){
                    ...
                    "<td>" + s.getHinhanh()+ "</td>" +
                    "<td>" + s.getGia()+ "</td>" +
                    "<td class='canphai'>" + s.getIdloaigiay()+ "</td>" + 
                        
                    "<td><button  name='id' type='submit' value= '" + s.getIdgiay()+ "'>Chỉnh sửa</button></td>");
                           
                
                           out.println("<td><button formaction='GiayDelServlet' name='id' type='submit' value='" 
                                   +s.getIdgiay()+"'onclick=\"return confirm('Ban co muon xoa ko');\">Xóa</a></td>");
                          
                           out.println("</tr>");
                
                }
          //  out.println(  "<td><img src='/image/boots5' ></img></td>" );
            out.println("</table>");

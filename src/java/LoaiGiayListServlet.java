 ...
 * @author Admin
 */
public class LoaiGiayListServlet extends HttpServlet {
...
"            <section>");
            ...
            out.println("<h2> Danh sách các loại giày trong cửa hàng</h2>");
            out.println("<input type=\"button\" value=\"Thêm mới\" onclick=\"location.href='LoaiGiayAddServlet';\">");
            out.println("<input type=\"button\" value=\"Tìm kiếm\" onclick=\"location.href='LoaiGiaySearchServlet';\">");
            DAOloaigiay dao = new DAOloaigiay();
             dao.ReadAll();
            ArrayList<loaigiay> list=dao.getList();
            
            out.println("<form action='LoaiGiayEditServlet' method= 'post'>");
             out.println("<table>");
             out.print("<th>IDLoạigiày</th>"
             + "<th>TênLoại</th>"
               + "<th>Chỉnhsửa</th>"
             + "<th>Xóa</th>");
            for (loaigiay s : list){
                    ...
                    out.println("<td>" + s.getIdloaigiay()+ "</td>" +
                            "<td>" + s.getTenloai()+ "</td>" +
                    "<td><button  name='id' type='submit' value= '" + s.getIdloaigiay()+ "'>Chỉnh sửa</button></td>");
                           
                
                           out.println("<td><button formaction='LoaiGiayDELServlet' name='id' type='submit' value='" 
                                   +s.getIdloaigiay()+"'onclick=\"return confirm('Ban co muon xoa ko');\">Xóa</button></td>");
                          
                    out.println("</tr>");
                
                }
            out.println("</table>");
               // out.println(s.getMasv()+s.getHo()+s.getTen()+s.getNamSinh()+s.getGioiTinh()+"<br>");
            out.println("</form>");
